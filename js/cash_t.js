...
var Record = {
    ...
    showData : function () 
    {
		...
		if(this.op_iotype=='1'){
			...
			listHTML += "<table  border='0' style='margin-top:-16px; width:100%;'>"
			 + '<tr>'
			
			 + '<th class="trans_id" scope="col" width="300px">提现流水号</th>'
			 + '<th class="out_ee" scope="col" width="200px">提现手续费</th>'
			
			 + '<th class="out_actual" scope="col" width="200px">实际到帐金额</th>'
			
			 + '<th class="out_money" scope="col" width="200px">提现金额</th>'
			 + '<th class="create_time" scope="col" width="300px">创建时间</th>'
			
			 + '<th class="state" scope="col" width="150px">用户状态</th>'
			  + "<th class='' ><pre>             </pre></th>"
			 + '</tr>';
		}
        ...
        for (var i = 0; i < len && i < this.limit; i++) {
		 ...
		 if(this.op_iotype=='1'){
			 ...
			 listHTML+=  "<td class='trans_id'>" + (until.getNodeValue("trans_id", nodes[i])|| '') + "</td>"
    ...
    query : function (page) 
    {
        ...
        var params = [
                      ...
                      { name: 'trade_time_beg', value: this.start_date }, 
                      { name: 'trade_time_end', value: this.end_date },
		...
		if(Record.op_iotype==1){
			...
			url='/cgi-bin/gy_query_cash_list.cgi';
	...
	submitExamineOk:function(){
			 this.showXMLMsgFlag = true;
                 ...
                 request.sendUpdate.call(this, '/cgi-bin/custom_cash_confirm.cgi', [
                 {
                    name : 'trans_id',
