...
var Record = {
	...
	getOpreation:function(state,amount,amount_ads,id){
		...
		if(state==1 && amount != amount_ads){
			str='<input id="'+(until.getNodeValue("trans_id", id)|| '')+'" class="my_btn" onclick="Record.openMydiv(event);" type="button" value="调账"/>';
    ...
    showData : function () 
    {
		...
		if(this.op_iotype=='1'){
			...
			listHTML += "<table  border='0' style='margin-top:-16px; width:1500px;'>"
			 + '<tr>'
			+ '<th class="channel" scope="col" width="80px">通道ID</th>'
			 + '<th class="trans_id" scope="col" width="100px">流水号</th>'
			 + '<th class="type" scope="col" width="70px">错账类型</th>'
			 + '<th class="check_date" scope="col" width="80px">对账日期</th>'
			 + '<th class="amount" scope="col" width="130px">交易金额（通道方）</th>'
			 + '<th class="amount_ads" scope="col" width="130px">交易金额（系统）</th>'
			 + '<th class="result" scope="col" width="130px">支付结果（通道方）</th>'
			 + '<th class="result_ads" scope="col" width="130px">支付结果（系统）</th>'
			
			 + '<th class="updater" scope="col" width="60px">调账人</th>'
			 + '<th class="update_time" scope="col" width="150px">调账时间</th>'
			 + '<th class="create_time" scope="col" width="150px">创建日期</th>'
...
//		}
	...
	getOutcomeFile_handle : function (xml) {
            ...
            var name = until.getNodeValue("file_name", xml); //返回信息
