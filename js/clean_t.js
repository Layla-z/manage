...
var Record = {
  ...
  //  datalistid : "listquery_data",  //显示数据区域
    ...
    showData : function () 
    {
    	...
    	//新增数字分页
    	var total = until.getNodeValue("total",this.xml);
            /*计算总页数*/
		...
		/*if(this.op_iotype=='1'){
			...
			listHTML += "<table  border='0' style='margin-top:-16px; width:1000px;'>"
			 + '<tr>'
			// + '<th class="other_name" scope="col" width="80px">用户名</th>'
			 + '<th class="trans_id" scope="col" width="180px">流水号</th>'
			...
			// + '<th class="bank_name" scope="col" width="100px">银行名</th>'
			 + '<th class="out_actual" scope="col" width="150px">实际到帐金额</th>'
			...
			// + '<th class="bank_branch_name" scope="col" width="150px">支行名</th>'
			 + '<th class="out_money" scope="col" width="100px">提现金额</th>'
			 + '<th class="create_time" scope="col" width="200px">创建时间</th>'
        ...
        for (var i = 0; i < len && i < this.limit; i++) {
		 ...
		 if(this.op_iotype=='1'){
			...
			// +"<td class='other_name'>" + (until.getNodeValue("other_name", nodes[i])|| '') + "</td>"           
...
/*       $(".tcdPageCode").createPage({
        ...
        /* $('income').onclick = function () 
		 ...
		 $('outcome_output').onclick= function(){
				 request.sendQuery.call(this, '/cgi-bin/down_custom_pay_list.cgi', [{name: 'state', value:'2'}],
